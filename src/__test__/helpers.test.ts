...
it('should convert Query Params to array', () => {
  expect(convertParamsToStateArray('all').length).toBe(5);
});

const filterArrayByStops = [
  {
    carrier: 'S7',
    price: 90263,
    segments: [
...
it('should filter bigger value of stops than we need to', () => {
  expect(filterTicketsByStops(filterArrayByStops, 2)?.length).toBe(1);
});
